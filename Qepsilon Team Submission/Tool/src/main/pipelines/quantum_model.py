...
PATH_TO_TRAINING = os.path.join(PROJECT_ROOT_PATH, 'Datasets', 'training_data.txt')
PATH_TO_TESTING = os.path.join(PROJECT_ROOT_PATH, 'Datasets', 'testing_data.txt')
...
def send_into_quantum_pipeline(sentence):
    ...
    grammar.draw(diagram, title='h', figsize=(12,3), fontsize=14)
...
def train_data():
    ...
    trainer.fit(train_dataset, test_dataset, evaluation_step=1, logging_step=20)
    test_acc = acc(model(test_circuits), val_labels)
